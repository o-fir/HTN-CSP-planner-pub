...
public class Layer {
    ...
    //decomposeLayer expands layer L into layer L+1. Return null if cannot be decomposed further (no methods)
    public Layer decomposeLayer(HTNProblem problem) {
        ...
        for (int i = 0; i < this.cells.size(); i++) {
            ...
            //expand the cell - !!REMEMBER!! every cell contains the possible actions/methods executable from this cell. 
            ...
            //every possible primitive action is executed in cell 0. The rest of the cells are populated with noop.
            for (int k = 0; k < initCell.getPrimitiveTasks().size(); k++) {
                //System.out.println("expanded primitive: " + initCell.getPrimitiveTasks().get(k));
                Task t = initCell.getPrimitiveTasks().get(k);
            ...
            for (Method m : initCell.getMethods()) {
                ...
                //fill the expansion with the items from the totally ordered list, fill the "leftovers" with noop
                for (int j = 0; j < expansion.size(); j++) {
                    if (j < tasks_totallyOrdered.size()) {
                        ...
                        if (t.isPrimtive()) {
                            expansion.get(j).addTask(t, tasks_totallyOrdered.get(j));
                        }
