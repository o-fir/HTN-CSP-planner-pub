    ...
    public static void formatPlan(List<Layer> network, HTNProblem problem) {
        ...
        for (int i = 0; i < finalLayer.getCells().size() - 1; i++) {
            ...
            if (valItem.getId() != 0 && found) {
                ...
                Task taskDEPREC = problem.getTasks().get(valItem.getId() - 1);
                Action action = problem.getActions().get(valItem.getId() - 1);

                taskOutput += action.getName() + " ";
                // step 1.2 - get task parameters
                int[] args = action.getInstantiations();
                for (int arg : args) {
